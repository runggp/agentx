...
- [ ] **Phase 1: Spend tracking** — Log token estimates per iteration, sess
...
- [x] **Phase 0f: First self-task** — `src/send_task.py` sends a `[task]` email to IMAP_USER via aiosmtplib. `async def send_task(cfg, description, spec)` reuses Config from listener. CLI: `uv run src/send_task.py "<description>" [spec.md]`. 6 tests in `src/tests/test_send_task.py`. Note: if AGENTX_ALLOWED_SENDERS is set, include SMTP_USER for self-tasks to be accepted.
...
- send-task.sh sends to IMAP_USER; if AGENTX_ALLOWED_SENDERS is non-empty it must include SMTP_USER for self-tasks to be accepted
